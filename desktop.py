"""smart-doc-parser 桌面应用 — PP-OCRv6 快速 + Unlimited-OCR 深度，自动组合。
...
class SmartDocParserApp:
    ...
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("Smart Doc Parser — PP-OCRv6 + Unlimited-OCR")
        self.root.geometry("900x700")
        self.root.minsize(700, 500)
        self.root.configure(bg="#1e1e2e")

        self.output_md_path: str | None = None
    ...
    def _setup_styles(self, style: ttk.Style):
        ...
        style.configure("Mode.TCombobox", font=("Microsoft YaHei UI", 10))
    ...
    def _build_ui(self):
        ...
        ttk.Label(main, text="PP-OCRv6 快速扫描 · Unlimited-OCR 深度解析 · 自动组合",
        ...
        ttk.Entry(file_frame, textvariable=self.file_var,
                  font=("Consolas", 10), state="readonly").pack(side="left", fill="x", expand=True, padx=(0, 8))
        ...
        ttk.Label(ctrl_frame, text="模式:", background="#1e1e2e",
                  foreground="#a6adc8", font=("Microsoft YaHei UI", 10)).pack(side="left", padx=(0, 4))
        self.mode_var = tk.StringVar(value="auto")
        mode_combo = ttk.Combobox(ctrl_frame, textvariable=self.mode_var,
                                   values=["auto", "quick", "deep"],
                                   state="readonly", width=8, style="Mode.TCombobox")
        ...
        ttk.Label(main, textvariable=self.status_var, style="Status.TLabel").pack(anchor="w", pady=(0, 8))
    ...
    def _run_pdf(self):
        try:
            ...
            def on_page_done(page, total, result):
                ...
                self.root.after(0, lambda: self.status_var.set(f"[{page}/{total}] {preview}"))
                self.root.after(0, lambda p=page, r=result: self._append_page(p, r))

            # PDF 默认 deep 模式（多页连续理解），quick/auto 模式走逐页
